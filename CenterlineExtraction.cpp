...
#include<string.h>
using namespace std;
unsigned int CenterlineExtraction(unsigned char* data, int width, int height, int max_segment_length, int smoothing_span, float **out_x, float **out_y, unsigned int **out_idx)
{
	...
	unsigned char* timg = new unsigned char[nelems];
	memset(timg, 0, sizeof(unsigned char)*nelems);
	unsigned char* t_ptr = timg;
	unsigned char* s_ptr = data;
	for (int hi = 0; hi < height; hi++, t_ptr += pwidth, s_ptr += width)
	{
		memcpy(t_ptr, s_ptr, width*sizeof(unsigned char));
	}


	// Convert Volume
	unsigned char* oimg = new unsigned char[nelems];

	// Extract segments
	Thinning2D<unsigned char>(pwidth, pheight, timg, oimg);
	vector<SSegment *> segs = ExtractSegmentsV(oimg, pwidth, pheight);
	delete[] oimg;
	delete[] timg;
